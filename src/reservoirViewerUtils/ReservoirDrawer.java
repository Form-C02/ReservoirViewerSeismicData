...
//        logFile.close();
        ...
        System.out.println("Highlighted Models:" + representativeModels);
        System.out.println("Number of highlighted Models:" + representativeModels.size());
        Dimension dimension;
        int kxSize = 0;
        int kySize = 0;
...
//                    double yIni = getYSmallMultiples(bottom, ky, ySize, maxJ, resgs) - resgs;
                    double xIni = getXSmallMultiples(kx, xSize, maxI, resgs);
                    double yIni = getYSmallMultiples(bottom, ky, ySize, maxJ, resgs);
...
//                    int currentClusterId = clusterIdMatrix[kx - 1][ky - 1];
        ...
        for (int i = 0; i < scaleNumberOfSteps; i++) {
            ...
            Rectangle scaleItemRect = new Rectangle(scaleColorBarStartX, y - interval, scaleColorBarWidth, interval);
...
//            System.out.println("rounded value="+value);
 ...
 /* Find the small value for numberOfRows and numberOfColumns:
    ...
    public void DrawPixelization(Configuracao configuracao, ReservoirMap res,
        ...
        double maxkxkySize = Math.max(kxSize, kySize);
//        double zoom = 0.8; // TODO should be automatically calculated based on the window height and width
        double zoom = 2; // TODO should be automatically calculated based on the window height and width
        double xSize = kySize / maxkxkySize * zoom;
        double ySize = kxSize / maxkxkySize * zoom;
        //int mainCellSize = size * 10;
        double gapSize = 1;
...
//                              // valueForPainting = Math.max(Math.min(value, maxValue), minValue);
        ...
        for (Well well : wellList) {
            ...
            for (Text label : wellLabels) {
                if (label.getBoundsInParent().intersects(t.getBoundsInParent())) {
                    hasCollision = true;
                }
            }
            if (hasCollision) {
...
//        PrintWriter logFileWriter = new PrintWriter(logFile);
    ...
    /**
     * Método que faz o processo de desenho para many models Levando em
     * consideração: - O tipo de visualização - O algoritmo de reordenação
     *
     * @param configuracao
     */
...
//            for (WellList wellList : estrategias) {
            ...
            if (configuracao.getClusteringConfig().getDistanceMatrix() != null) {
...
//                    }
     ...
     * @param configuracao
     *
     * @return Uma Lista contendo os modelos representativos
     */
    private List<Integer> getRepresentativeModelsList(Configuracao configuracao) {
        ...
        try {
            fileName = configuracao.getRoot() + File.separator + configuracao.getHighlightedModels();
            ...
            System.out.println("I/O Exception related to file " + fileName);
        }
