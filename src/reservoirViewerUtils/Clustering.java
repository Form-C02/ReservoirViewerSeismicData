    ...
    public ClusteringData getClustering (Instances data, Configuracao configuracao) {
        ...
        switch (configuracao.getClusteringConfig().getXsiType().toUpperCase()) {
            ...
            case "XMEANS":
                ...
                // MIN E MAX para a execução do XMEANS
                min = configuracao.getClusteringConfig().getMin().toString();
                max = configuracao.getClusteringConfig().getMax().toString();
                
                Options = "-I 100 -L " + min + " -H " + max ;
                System.out.println("Running XMeans with option "+Options);
                try {
            ...
            case "KMEANS":
                ...
                Options="-N 4";
                System.out.println("Running KMeans with option "+Options);
                try {
                    clusterer.setOptions(weka.core.Utils.splitOptions(Options));
                    clusterer.buildClusterer(data);
                    ClusteringData clusteringData = new ClusteringData(clusterer, data);
                    // TODO INSPECIONAR QUANTIDADE DE CLUSTERS RETORNADA. KMEANS ASSIM COMO O XMEANS ESTÁ RETORNANDO APENAS 2 CLUSTERS.
                    return clusteringData;
